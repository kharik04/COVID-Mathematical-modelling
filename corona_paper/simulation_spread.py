...
class Simulation:
    ...
    def __init__(self, n, vr, p, types, dt, sim_time, sicknessperiod, radius=0.01,  styles=None):
...
if __name__ == '__main__':
    radii = 0.02
    #collision rate = sqrt(2)*d*v*poh*N
    #v=r/(sqrt(2)*d*poh*N)
    #stylesS = {'edgecolor': 'C0', 'linewidth': 2, 'fill': None}
    types = [{"type":"S", "num":95}, {"type":"I", "num":5}, {"type":"R", "num":0}]
    ...
    sim_time=50
    rate = 50
    initvel = rate/(2**0.5*radii*2*nparticles*nparticles/1)
    sim = Simulation(nparticles, initvel, p, types, dt, sim_time, sicknessperiod, radii)
